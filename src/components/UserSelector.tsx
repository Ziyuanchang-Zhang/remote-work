...
const providerConfig = {
  ...
  order:['openai','ollama'],
  hidden:['google','openrouter','bedrock','azure']
};
const getDisplayProviders = (providers: Provider[]): Provider[] => {
  return providers
    .filter(p => !providerConfig.hidden.includes(p.id))
    .sort((a, b) => {
      const aIndex = providerConfig.order.indexOf(a.id);
      const bIndex = providerConfig.order.indexOf(b.id);
    ...
    <div className="flex flex-col gap-3">
      <div className="space-y-4">
        ...
        <div>
          ...
          >
            {getDisplayProviders(modelConfig?.providers || []).map((providerOption) => (
              <option key={providerOption.id} value = {providerOption.id}>
                {providerConfig.displayNames[providerOption.id] || providerOption.name}
              </option>
            ))}
          </select>
        </div>
