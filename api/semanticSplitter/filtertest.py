...
# 获取当前脚本所在目录的上级目录（假设主脚本和 semanticSplitter 平级）
root_path = os.path.dirname(os.path.abspath(__file__))  
sys.path.append(root_path)
from adalflow.core.types import Document
from semanticSplitter.ParallelHybridTextSplitter import MultiLanguageTextSplitter
from semanticSplitter.function_filter import FunctionFilterProcessor
...
for i, chunk in enumerate(split_chunks[:5]):
    print(f"\nChunk {i+1}:")
    print(f"文件路径: {chunk.meta_data.get('file_path', 'unknown')[:50]}...")  # 截断长路径
...
# 打印部分头文件函数
print("\n部分头文件函数：")
for func in list(processor.header_functions)[:5]:  # 只显示前5个
    print(f"- {func['name']} (文件: {os.path.basename(func['file_path'])})")

# 打印部分跨模块调用
print("\n部分跨模块调用：")
for call in list(processor.cross_module_calls)[:5]:  # 只显示前5个
