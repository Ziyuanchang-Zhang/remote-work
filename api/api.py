...
class ModelConfig(BaseModel):
    ...
    defaultModel: str = Field(..., description="ID of the default model")
...
@app.get("/vllm/status")
async def get_vllm_state():
    try:
        vllm_server_url = "http://10.44.151.53:8111"

        async with httpx.AsyncClient(timeout = 10.0) as client:
            try:
                response = await client.get(f"{vllm_server_url}/metrics")
                ...
                if response.status_code == 200:
                    ...
                    for line  in metrics_text.split('\n'):
                        ...
                        if 'vllm:num_requests_running' in line:
                            match = re.search(r'vllm:num_requests_running\{.*?\}\s+(\d+(?:\.\d+)?)',line)
                            logger.info(f"================{line}")
                            if match:
                                processing = int(float(match.group(1)))
                        elif 'vllm:num_requests_waiting' in line:
                            match = re.search(r'vllm:num_requests_waiting\{.*?\}\s+(\d+(?:\.\d+)?)',line)
                            logger.info(f"================{line}")
                            if match:
                                queued = int(float(match.group(1)))
                    result = {"processing": processing, "queued": queued}
                    logger.info(f"vLLM Statue: {result}")
                    return result
                else:
                    logger.error(response)
                    return {"processing": 'e', "queued": 'e'}
            except httpx.RequestError as e:
                return {"processing": 'e', "queued": 'e'}
            
    except Exception as e:
        logger.error(f"获取vLLM状态发生错误： {str(e)}")
        return {"processing": 'e', "queued": 'e'}
...
async def get_model_config():
    ...
    try:
        ...
        default_model = configs.get("default_model","Qwen3-235B-A22B-Thinking-2507-FP8")
