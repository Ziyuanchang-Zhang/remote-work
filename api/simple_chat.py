...
included_files = None
import asyncio
retriever_cache: Dict[str, RAG] = {}
retriever_locks: Dict[str, asyncio.Lock] = {}
async def get_retriever(repo_url: str,request: ChatCompletionRequest):
    logger.info(f"repo_url in retriever_cache:{repo_url in retriever_cache} + {repo_url} + {retriever_cache.keys()}")
    if repo_url in retriever_cache:
        return retriever_cache[repo_url]
    # 获取当前repo对应的锁
    if repo_url not in retriever_locks:
        retriever_locks[repo_url] = asyncio.Lock()
    async with retriever_locks[repo_url]:
        if repo_url in retriever_cache:
            return retriever_cache[repo_url]
        
        logger.info(f"Creating retriever for {repo_url}")
        ...
        logger.info(f"============================= rag already init ====================")
        retriever_cache[repo_url] = rag
        return rag
...
async def chat_completions_stream(request: ChatCompletionRequest):
    ...
    try:
        ...
        try:
            ...
            request_rag = await get_retriever(request.repo_url, request)
        ...
        request_rag.memory.current_conversation.dialog_turns.clear()
...
- Use markdown formatting to improve readability
        ...
        turn_memory = freshMemory.call()
        conversation_history = ""
        for turn_id, turn in turn_memory.items():
        ...
        if request.provider == "ollama":
            ...
            logger.info(f"simple_chat:{request.model}")
            # Initialize Openai client
            model_api_key=str(request.model)+"_API_KEY"
            model_base_url=str(request.model)+"_BASE_URL"
            api_key=os.getenv(model_api_key)
            base_url=os.getenv(model_base_url)
            model = OpenAIClient(api_key=api_key,base_url=base_url)
            model_kwargs = {
                "model": request.model,
                "stream": True,
                "extra_body": {
            ...
            try:
                if request.provider == "ollama":
                    ...
                    try:
                        ...
                        logger.info(f"调用前时间花费========================================{time.time() - start}")
                        response = await model.acall(api_kwargs=api_kwargs, model_type=ModelType.LLM)
                        logger.info("==========获取信息==========")
                        # 心跳和连接状态管理
                        ...
                        async for chunk in response:
                            ...
                            if not heartbeat_sent:
                                ...
                                heartbeat_sent = True
                                logger.info("=====heartbeat sent, connection established======")
                        ...
                        error_data = {
                            "type": "error",
                            "data": f"Error with Openai API: {str(e_openai)}\n\nPlease check that you have set the OPENAI_API_KEY environment variable with a valid API key.",
                            "timestamp": int(time.time() * 1000)
                        }
