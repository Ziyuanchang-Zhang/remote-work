...
9. For pipe characters (|) in text, write them directly without escaping them"""
    ...
    def prepare_retriever(self, repo_url_or_path: str, type: str = "github", access_token: str = None,
        ...
        start = time.time()
        logger.info(f"==========rag.py prepare_database 开始=========={start}")
        ...
        logger.info(f"==========rag.py prepare_database 结束=========={time.time() - start}")
        ...
        try:
            ...
            start1 = time.time()
            logger.info(f"==========rag.py FAISSRetriever 开始=========={start1}")
            ...
            logger.info(f"==========rag.py FAISSRetriever 结束=========={time.time() - start1}")
            start2 = time.time()
            logger.info(f"==========rag.py BM25Retriever 开始=========={start2}")
            self.retriever_bm25=BM25Retriever(top_k=20,documents=self.transformed_docs,document_map_func=lambda doc:doc.text)
            logger.info(f"==========rag.py FAISSRetriever 结束=========={time.time() - start2}")
    ...
    def call(self, query: str, language: str = "en") -> Tuple[List]:
        ...
        try:
            ...
            logger.info("retriver searching")
            start = time.time()
            retrieved_documents = self.retriever(query)
            retrieved_documents_bm25 = self.retriever_bm25(query)
            logger.info(f"================================{time.time() - start}")
            logger.info("bm25_retriver")

            indices1 = retrieved_documents[0].doc_indices
            logger.info(f"Vector retriever count: {len(indices1)}")
            indices2 = retrieved_documents_bm25[0].doc_indices
            logger.info(f"BM25 retriever count: {len(indices2)}")

            source_dict = {}
            for idx in indices1:
                source_dict.setdefault(idx, []).append("vector")
            for idx in indices2:
                source_dict.setdefault(idx, []).append("bm25")
            ...
            retrieved_documents[0].doc_indices = merged_indices
            retrieved_documents[0].documents = [
                self.transformed_docs[doc_index] 
                for doc_index in merged_indices
            ]
            merged_documents = retrieved_documents[0].documents

            
            query_result = {
                "timestamp": datetime.datetime.now().isoformat(),
                "query": query,
                "retriever": "hybrid retriever(FaissRetriever and bm_25)",
                "merged_documents": []
            }

            for doc_index, doc in zip(merged_indices, merged_documents):
                
                sources = source_dict.get(doc_index, [])
                ...
                doc_dict = {
                    ...
                    "meta_data": {
                        ...
                        "type": doc.meta_data.get("type","unknown"),
                        "is_code": doc.meta_data.get("is_code","unknown"),
                        "is_implementation": doc.meta_data.get("is_implemention","unknown"),
                        "title": doc.meta_data.get("title","unknown"),
                        "token_count": doc.meta_data.get("token_count","unknown")
                    }
                }
                query_result["merged_documents"].append(doc_dict)

            filename = f"query_result/query_results_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            with open(filename, 'w', encoding='utf-8') as f:
                json.dump(query_result, f, indent=2, ensure_ascii=False)
                
            logger.info(f"{len(merged_documents)}")
